...
module VeracodeApiBase
	...
	def write(data, to_file:)
		...
		f = File.open "../logs/#{to_file}.json", 'a'
...
module VeracodeApiScan
	...
	def validate_existance(of:)
		...
		if app_list.include? "#{of}"
			...
			#write create_app_result, to_file: "profile_creation_log"
	...
	def submit_scan(hostname, archive_path)
		...
		#NOTE: curl must be used here because of a bug in the Veracode api. Pure Ruby cannot be used while this bug is present.
		...
		puts upload_result
		#write upload_result, to_file: "#{app_id}_upload_log"
		...
		#write prescan_submission_result, to_file: "#{app_id}_prescan_submission_log"
	end
end
